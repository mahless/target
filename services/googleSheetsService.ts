...
export const GoogleSheetsService = {
     ...
     * @param branchId - Branch ID
     * @param collectionId - Unique collection ID for tracking (optional)
     * @param isElectronic - Whether the payment is electronic (optional)
     * @param electronicMethod - The method used for electronic payment (optional)
    ...
    /**
     * Get a single entry by its ID (for public tracking page).
     * 
     * @param entryId - The unique entry ID
     * @returns ServiceEntry or null if not found
     */
    async getEntryById(entryId: string): Promise<ServiceEntry | null> {
        if (!GOOGLE_SCRIPT_URL) return null;
        try {
            const entries = await this.getData(SHEET_NAMES.ENTRIES) as ServiceEntry[];
    ...
    /**
     * Archive "Delivered" data within a date range.
     * 
     * @api POST ?action=archiveData
     * @param startDate - Range start (yyyy-MM-dd)
     * @param endDate - Range end (yyyy-MM-dd)
     * @param role - User role (must be admin)
     * @returns BackendResponse { status, movedCount, message }
     */
    async archiveData(startDate: string, endDate: string, role: string): Promise<{ success: boolean; movedCount: number; message: string }> {
        if (!GOOGLE_SCRIPT_URL) return { success: false, movedCount: 0, message: 'URL missing' };
        try {
            const response = await this.fetchWithRetry(`${GOOGLE_SCRIPT_URL}?action=${API_ACTIONS.ARCHIVE_DATA}&role=${encodeURIComponent(role)}`, {
                method: 'POST',
                headers: { "Content-Type": "text/plain;charset=utf-8" },
                body: JSON.stringify({ startDate, endDate })
            }, NETWORK.TIMEOUT_LONG * 2);
            ...
            return { success: false, movedCount: 0, message: 'خطأ في تنفيذ عملية الأرشفة' };
        }
    },
     ...
     * 
     * @api GET ?action=searchArchives
     * @param query - Search text
     * @returns Array of any objects
     */
    async searchArchives<T>(query: string): Promise<T[]> {
        if (!GOOGLE_SCRIPT_URL) return [];
        try {
            const response = await this.fetchWithRetry(`${GOOGLE_SCRIPT_URL}?action=${API_ACTIONS.SEARCH_ARCHIVES}&query=${encodeURIComponent(query)}&t=${Date.now()}`);
            const json = await response.json();
            if (json && !Array.isArray(json) && (json as BackendResponse).status === STATUS.ERROR) {
                return [];
            }
