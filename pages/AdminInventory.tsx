...
const AdminInventory: React.FC<AdminInventoryProps> = ({ stock, onRefresh, onDeleteStock, isSyncing, userRole, username, isSubmitting = false, startSubmitting, stopSubmitting, branches }) => {
 ...
 <div>
 <label className="text-[10px] font-black text-[#01404E]/40 block mb-3 mr-1 uppercase">الباركودات (رقم في كل سطر)</label>
 <textarea
 value={newBarcodes}
 ...
 <p className="text-[10px] font-black text-[#01404E]/40 mb-1.5 border-b border-[#01404E]/5 pb-2 uppercase ]">{b.name}</p>
 ...
 className="text-[#00A6A6] hover:underline font-black group-hover:scale-110 transition-transform inline-block text-sm md:text-base"
 ...
 <span className={`px-3 py-1 rounded-xl text-[9px] font-black uppercase ${(item.category || (item as any).Category) === 'فوري' ? 'bg-red-50 text-red-600' :
 ...
 <td className="py-2 px-6 text-center">
 <span className={`px-3 py-1 rounded-xl text-[9px] font-black uppercase ${(item.status || (item as any).Status) === 'Used' ? 'bg-green-50 text-green-600' :
 ...
 <td className="py-2 px-4 text-center text-gray-400 text-xs md:text-sm">
