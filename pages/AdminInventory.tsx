...
const AdminInventory: React.FC<AdminInventoryProps> = ({ stock = [], onRefresh, isSyncing, userRole }) => {
    ...
    const clearLocalStock = () => {
        if(confirm('هل أنت متأكد من مسح الذاكرة المؤقتة وإعادة التحميل؟')) {
            ...
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex gap-2">
                  <button
                      ...
                      className="bg-red-50 text-red-600 px-4 py-3 rounded-2xl font-black text-xs hover:bg-red-100 transition-all flex items-center gap-2"
                      title="مسح الكاش المحلي وإعادة التحميل"
                  >
                      <RefreshCw className="w-4 h-4" /> إصلاح / مسح
                  </button>
                  <button
                      onClick={onRefresh}
                      disabled={isSyncing}
