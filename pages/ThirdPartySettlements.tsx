...
const ThirdPartySettlements: React.FC<ThirdPartySettlementsProps> = ({
    entries, onUpdateEntry, onAddExpense, branchId, currentDate, username, isSyncing, onRefresh, isSubmitting = false, branches
}) => {
            ...
            content: (
                <div className="space-y-3 text-right">
                    <div className="grid grid-cols-2 gap-2">
                        ...
                        <div className="bg-gray-50 p-2 rounded-xl border border-gray-100">
                            <span className="text-[10px] text-gray-400 font-black block mb-0.5">المورد</span>
                            <p className="font-black text-blue-800 text-xs">{entry.thirdPartyName}</p>
                        </div>
                        <div className="bg-gray-50 p-2 rounded-xl border border-gray-100">
                            <span className="text-[10px] text-gray-400 font-black block mb-0.5">تكلفة المورد</span>
                            <p className="font-black text-blue-800 text-xs">{entry.thirdPartyCost} ج.م</p>
                        </div>
                        <div className="bg-gray-50 p-2 rounded-xl border border-gray-100">
                            <span className="text-[10px] text-gray-400 font-black block mb-0.5">تاريخ التسجيل</span>
                            <p className="font-black text-gray-800 text-xs">{entry.entryDate}</p>
                        </div>
                    </div>
                </div>
            ),
    ...
    const filteredEntries = useMemo(() => {
        return entries.filter(e => {
            const isUnpaidThirdParty = e.hasThirdParty && (Number(e.thirdPartyCost) || 0) > 0 && !e.isCostPaid && e.status !== 'cancelled';
        ...
        <div className="p-3 md:p-5 space-y-4 text-right">
            <div className="bg-white p-4 rounded-3xl shadow-sm border border-gray-100 space-y-3">
                <div className="flex items-center gap-3 border-b border-gray-50 pb-3 mb-1">
                    ...
                    <h3 className="text-lg font-black text-gray-800">تسويات الموردين (الطرف الثالث)</h3>
                </div>
                                    ...
                                    <tr key={entry.id} className="hover:bg-blue-50/20 transition-all group">
                                        <td className="py-3 px-6 font-black">
                                            ...
                                            <div className="flex gap-2 items-center mt-0.5">
                                                <span className="text-[9px] text-blue-800 bg-blue-50 px-1.5 py-0.5 rounded-md font-black">المورد: {entry.thirdPartyName}</span>
