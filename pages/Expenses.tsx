...
import { toEnglishDigits, useDebounce, searchMultipleFields, normalizeArabic } from '../utils';
...
const Expenses: React.FC<ExpensesProps> = ({ expenses, entries, expenseCategories, onAddExpense, branchId, currentDate, username, isSubmitting = false, branches, onDeleteExpense, userRole }) => {
 ...
 className="flex-1 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-2xl font-black shadow-lg shadow-blue-600/20 active:scale-95 transition-all text-xs"
 >
 ...
 className="px-6 py-3 rounded-xl font-bold text-gray-500 bg-gray-50 border border-gray-100 hover:bg-gray-100 transition-all active:scale-[0.98] text-xs"
 >
 إغلاق
 </button>
 </div>
 ...
 <div>
 <label className="block text-[10px] font-black text-[#01404E]/40 mb-1.5 ml-1 uppercase">القيمة المالية</label>
 ...
 <div>
 <label className="block text-[10px] font-black text-[#01404E]/40 mb-1.5 ml-1 uppercase">بيان التفاصيل</label>
 <textarea
 value={notes}
 ...
 <span className="text-[10px] font-black text-white/40 uppercase ] mb-2">إجمالي مصروفات اليوم</span>
 ...
 <span className="text-2xl md:text-[28px] font-black text-white">{totalExpenses.toLocaleString()}<span className="text-sm md:text-lg mr-2 opacity-50">ج.م</span></span>
 </div>
 </div>
 ...
 <span className="text-gray-300 font-black italic">
 {debouncedSearchTerm ? 'لا توجد نتائج تطابق بحثك' : 'لم يتم تسجيل أي مصروفات اليوم'}
 </span>
 </div>
 </td>
 </tr>
 ...
 <span className="bg-red-50 text-red-600 px-3 py-1 rounded-xl text-[9px] font-black uppercase">{expense.category}</span>
 {linkedEntry && (
 <button
 type="button"
 onClick={(e) => {
 e.preventDefault();
 e.stopPropagation();
 showCustomerDetails(linkedEntry);
 }}
 ...
 <td className="py-6 px-6 text-red-600 font-black text-lg text-center">{expense.amount.toLocaleString()}<span className="text-[10px] mr-1 opacity-50 uppercase">ج.م</span></td>
 <td className="py-6 px-6">
 ...
 >
 تحميل المزيد ({filteredExpenses.length - visibleCount} متبقي)
 </button>
 </div>
 )}
 </div>
