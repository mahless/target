...
const AttendanceDashboard: React.FC = () => {
 ...
 return (
 <div className="px-4 pt-1 md:px-8 md:pt-2 pb-8 space-y-4 transition-opacity animate-premium-in" dir="rtl">
 ...
 <div className="w-9 h-9 bg-[#00A6A6]/10 rounded-2xl flex items-center justify-center text-[#00A6A6] shadow-sm border border-[#00A6A6]/20">
 <Users className="w-5 h-5" />
 </div>
 <div>
 <h2 className="text-lg font-black">بيانات الحضور والانصراف</h2>
 </div>
 </div>

 <div className="flex bg-white/50 p-1.5 rounded-[1.5rem] border border-white/40 shadow-premium gap-3 items-center">
 <input
 type="month"
 value={selectedMonth}
 onChange={(e) => setSelectedMonth(e.target.value)}
 className="p-2 border border-[#01404E]/10 rounded-xl bg-[#01404E]/5 text-[#01404E] font-black text-sm outline-none focus:bg-white transition-all shadow-sm"
 />
 ...
 <button
 onClick={fetchReport}
 className="p-3 bg-[#01404E]/5 border border-[#01404E]/10 rounded-xl hover:bg-white hover:shadow-lux transition-all shadow-sm active:scale-95"
 ...
 <tr className="bg-[#01404E] text-white/50 text-xs font-black ] uppercase border-b border-white/5">
 ...
 <span className="inline-flex items-center gap-1 px-1 rounded-lg text-[10px] font-black uppercase bg-red-50 text-red-600 border border-red-100">
 {STATUS.ABSENT}
 </span>
 ) : (
 <span className="inline-flex items-center gap-1 px-1 rounded-lg text-[10px] font-black uppercase bg-green-50 text-green-600 border border-green-100">
 {STATUS.PRESENT}
 </span>
 )}
 </td>
 <td className="py-2 px-6 text-center whitespace-nowrap font-mono font-black text-[#01404E]/70 text-xs">
 {user.checkIn !== '-' ? user.checkIn : '-'}
 </td>
 <td className="py-2 px-6 text-center whitespace-nowrap font-mono font-black text-[#036564] text-xs">
 {user.checkOut !== '-' ? user.checkOut : '-'}
 </td>
 <td className="py-2 px-6 text-center whitespace-nowrap bg-blue-50/10">
 <span className="font-black text-[#00A6A6] font-mono text-lg">{user.todayTotal}h</span>
 </td>
 <td className="py-2 px-6 text-center">
 <div className="flex justify-center">
 {selectedUser === user.id ? (
 <ChevronUp className="w-4 h-4 text-[#00A6A6]" />
 ) : (
 ...
 <tr className="bg-[#01404E]/5 text-[#01404E]/60 font-black text-xs uppercase">
 ...
 <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase ${log.type === STATUS.CHECK_IN
