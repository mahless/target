...
const AttendanceDashboard: React.FC = () => {
    ...
    const fetchReport = async () => {
        setLoading(true);
        const data = await GoogleSheetsService.getHRReport();
        setReport(data);
        setLoading(false);
    };
    ...
    const handleUserClick = async (username: string) => {
        ...
        setLogsLoading(true);
        const logs = await GoogleSheetsService.getUserLogs(username);
        setUserLogs(logs);
            ...
            <div className="max-w-7xl mx-auto mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">
                        ...
                        لوحة بيانات الحضور والانصراف
                    </h1>
                    <p className="text-gray-500 mt-1 font-medium">متابعة أداء الموظفين والساعات الشهرية</p>
                </div>
                            ...
                            <table className="w-full text-right border-collapse">
                                <thead>
                                    <tr className="bg-gray-50 border-b border-gray-100">
                                        ...
                                        <th className="px-6 py-4 text-xs font-black text-gray-500 uppercase tracking-wider text-center bg-blue-50/50">إجمالي الشهر</th>
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    {filteredReport.length > 0 ? filteredReport.map((user) => (
                                        <React.Fragment key={user.name}>
                                            <tr
                                                className={`hover:bg-blue-50/30 cursor-pointer transition-colors ${selectedUser === user.name ? 'bg-blue-50/50' : ''}`}
                                            ...
                                            >
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-center font-mono font-bold text-blue-600">
                                                    {user.todayStatus !== 'غائب' ? user.todayStatus : '-'}
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap text-center bg-blue-50/50">
                                                    <span className="font-black text-blue-700 font-mono text-lg">{user.totalMonth}h</span>
                                                </td>
                                            ...
                                            {selectedUser === user.name && (
                                                <tr>
                                                    <td colSpan={5} className="px-6 py-4 bg-gray-50/80 border-t border-gray-100">
                                                        <div className="max-w-4xl mx-auto py-4">
                                                            <div className="flex items-center justify-between mb-4">
                                                                <h3 className="text-sm font-black text-gray-800 flex items-center gap-2">
                                                                    <Clock className="w-4 h-4 text-blue-600" />
                                                                    تفاصيل سجلات الحركات لشهر {new Date().getMonth() + 1}
                                                                </h3>
                                                            </div>
