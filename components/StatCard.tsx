...
const StatCard = React.memo(({ title, value, icon, color, footer, gradient }: StatCardProps) => {
  ...
  return (
    <div className={`relative overflow-hidden bg-gradient-to-br ${bgClass} p-2 rounded-2xl shadow-lux transition-all duration-500 hover:-translate-y-1 hover:scale-[1.01] hover:shadow-2xl group animate-premium-in`}>
      ...
      <div className="relative z-10 flex flex-col h-full justify-between gap-0">
        <div className="flex flex-col">
          <span className="text-[9px] md:text-[10px] font-black text-white/70 uppercase leading-none mb-1">{title}</span>
          <div className="flex items-baseline gap-1">
            <span className="text-xl md:text-2xl font-black text-white leading-none">{value?.toLocaleString('en-US')}</span>
            <span className="text-[9px] font-bold opacity-60 text-white">ج.م</span>
          </div>
        </div>
        <div className="pt-1 mt-1 border-t border-white/5 text-[8px] md:text-[9px] text-white/50 font-bold uppercase flex items-center gap-1 leading-none">
